...
Optimize the UDP/DMR routing and SDS ingestion paths so they allocate 0 managed bytes per packet after startup and warm-up within configured capacities. SMS completion and MQTT are best-effort allocation targets.
...
- `make test` passes 25/25 tests in Release mode.
...
- Corrected `docs/components/mqtt.adoc` to match the implemented topic paths (`calls`, `roaming`, and `sms`).
